...
//output files
static const char pos_file_name_verlet[] = "pos_verlet.dat";
...
int main(void)
{
	...
	delta_t[0] = 1;
	double step = pow(1.0e-4/delta_t[0],1./99.);
	...
	FILE *pos_file_verlet = fopen(pos_file_name_verlet, "w");
	...
	for(int j = 0; j < max_iter; j++)
	{
		...
		while (t <= Tmax)
		{
			...
			verlet_step(t, current_step, y_ve, pendulums_ode, dimension, NULL);

			t += current_step;
		}
		
		fprintf(pos_file_verlet, "%g\t%g\n", delta_t[j], res_ve);
